(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('bookmakerApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-bookmakerApp-error');
                var entityKey = httpResponse.headers('X-bookmakerApp-params');
                if (errorHeader) {
